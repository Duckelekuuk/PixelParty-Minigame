...
public class LobbyScoreBoard {
    ...
    public LobbyScoreBoard(PixelPartyFrame plugin) {
        ...
        this.scoreboard =
                new ScoreboardBuilder("Lobby", DisplaySlot.SIDEBAR, "&6&lPixel&e&lParty")
                        ...
                        .addScore("game", ChatColor.GREEN + (plugin.getGameManager().getCurrentGame() == null ? "No game" : plugin.getGameManager().getCurrentGame().getGameName()), ChatColor.DARK_BLUE.toString(), "", 4)
    ...
    public void updateScoreboard(int timeLeft) {
        ...
        scoreboard.getTeam("game").setPrefix(plugin.getGameManager().getCurrentGame() == null ? ChatColor.RED + "No game" : ChatColor.GREEN + plugin.getGameManager().getCurrentGame().getGameName());
